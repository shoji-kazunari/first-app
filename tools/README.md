...
各ページのcanonical（検索エンジンに「このページの正式なURLはこれ」と伝えるタグ）は、
`assets/js/core/config.js` の `siteBaseUrl` から組み立てている。
末尾のスラッシュは付けない。

独自ドメインへ移したら、この1行を書き換えて
`node tools/build-machine-pages.js` を実行すれば、TOP・機種ページ・
プライバシーポリシーのcanonicalがまとめて切り替わる。

ページごとに手書きしていたときは、公開先がサブパス配下（`/first-app/`）である
ことを忘れて `/machines/<slug>/` と書いており、実在しないURLを正式なURLとして
宣言してしまっていた。1箇所にまとめたのはこれを繰り返さないため。
