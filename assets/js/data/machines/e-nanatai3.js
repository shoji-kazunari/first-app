...
//
// 【出玉0個での実装（2026-09-02改訂）】
// 当初はstateEngineのonHit.outcomesが出玉が正の数であることを前提としていた
// ため、「チャレンジ当選＝出玉無し」をそのまま0個では表現できず、この機種で
// 最小の単位である3R(実獲得420個)を代表値として仮に付与していた（本来より
// 僅かに出玉が多く出る近似）。その後machineValidator.jsのballsチェックを
// 「明示的なオーバーライドとしての0」を許可するよう緩和したため、実機通り
// balls: 0で表現し直した。roundsは実際にはラウンド消化を伴わない当たりなので
// 意味を持たない値だが、validatorがrounds自体は正の整数を要求するため、
// 最小値の1を形式的に入れている（画面には「SEVEN RUSH突入」というresultNote
// だけが出て、＜1R獲得＞という表記やラウンド数自体は表示されない）。
//
// 【出玉は「実獲得個数」を採用】
...
PachiSim.machineRegistry.register({
  ...
  rules: [
    ...
    "SEVEN RUSH継続率：約75%",
    "通常時の大当り振り分け（ヘソ入賞時）：3R・実獲得約420個で必ずSEVEN RUSHチャレンジ(時短100回)へ",
    "SEVEN RUSHチャレンジ中の当選：出玉無しで必ずSEVEN RUSHへ突入",
