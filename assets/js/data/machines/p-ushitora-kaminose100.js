...
//
// 【「二体で最強BONUS」の内訳（2026-09-03追記）】
// 1geki.jpには「約2000個以上（10R×2以上）」～「最大約5000個+α（10R×5+α）」
// という幅しか掲載が無く、10R×2/3/4/5各段の個別の振り分け（重み）の記載が
// 無かった。1geki.jpは本機種の標準ソースとして使い続けつつ、この内訳だけは
// 別の解析サイト（chonborista.com、およびp-world.co.jp／依頼者からの
// スクリーンショットで提供）で補完した。両サイトとも独立に同じ内訳
// （2000個50%・3000個25%・4000個12.5%・5000個+α12.5%）を示しており、
// 数値が一致することを確認済み。5000個+α（12.5%）はそれ以上の細かい内訳が
// 無いため、代表値として5000個をそのまま採用した（実際の期待値はこれより
// 僅かに高くなる可能性がある）。
//
// 【出玉は「実獲得個数」を採用】
// スペック表の実獲得個数をそのまま使用（10R: 約1000個/実獲得900個、
// 3R: 約300個/実獲得270個、比率9/10）。「二体で最強BONUS」の各段
// （2000/3000/4000/5000個）にも同じ比率を適用した
// （2000→1800、3000→2700、4000→3600、5000→4500）。
...
PachiSim.machineRegistry.register({
  ...
  rules: [
    ...
    "真うしとらRUSH中の当選振り分け（電チュー入賞時）：10R・実獲得約900個で継続が50.0%、二体で最強BONUSで継続が50.0%（内訳：10R×2・実獲得約1800個が25.0%、10R×3・実獲得約2700個が12.5%、10R×4・実獲得約3600個が6.25%、10R×5+α・実獲得約4500個以上が6.25%）",
