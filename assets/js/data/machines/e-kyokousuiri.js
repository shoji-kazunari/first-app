...
//     （ストック4個固定でスタート）、18%でお願い玉がVストック化し裏ご褒美RUSHへ
//     （こちらもストック4個固定）。※82/18の比率は1geki.jp由来の実機データをそのまま
//     採用（ユーザーからの新指示に含まれていなかったため、確認済みの値をデフォルトに
//     採用。違っていれば教えてほしい）。
...
PachiSim.machineRegistry.register({
  ...
  rules: [
    ...
    "琴子のご褒美RUSH中、お願い玉が残り3個以上での当たりは、82%で継続（約3000個・ストック4個固定）、18%でVストック化し裏ご褒美RUSHへ（約3000個・ストック4個固定）",
