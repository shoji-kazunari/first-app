...
// 「敗北した場合でもシャーマンファイト（LT）突入となる」という記載のとおり、
// 起きパチョモードは20回消化後（または途中のシャーマンバトル勝利）で必ず
// シャーマンファイトへ移行する。ヒットしてもしなくても最終的に同じ状態へ着地する
// 構造のため、20回のcountDownのonHit（1/348.6でシャーマンバトル発生、勝率は
// シャーマンファイトと同じ53%と仮定）とonExhausted（どちらもシャーマンファイトへ）
// で表現した。起きパチョモード中のシャーマンバトル勝利時の出玉振り分けは個別の
// 円グラフが無いため、シャーマンファイトと同じ4パターンの比率を流用した（未確認の
// 仮定として明記）。
//
...
PachiSim.machineRegistry.register({
  ...
  states: {
    ...
    okipachoMode: {
      ...
      onExhausted: { nextState: "shamanFight", tag: "okipachoEnd", resultLabel: "シャーマンファイト" },
    },
