...
//
// 【運命分岐突入は出玉0個（2026-09-02訂正）】
// 当初は「出玉の記載無し」の枝をstateEngineの出玉0個不可の制約
// （e-nanatai3.jsのチャレンジ→RUSHと同じ問題）のため、この機種最小単位の
// 約400個を代表値として付与していた。依頼者の実機プレイ経験から、運命分岐
// 突入自体は「当たりだが出玉は無い」イベントで、その直後に運命分岐状態の
// 抽選（下記50%/29%/21%）で初めて出玉が確定するという構造だと訂正を受けた
// （1geki.jpの円グラフ「10R×2・約3000個→復讐RUSHへ」等は、運命分岐突入
// から確定までの累計値であり、突入自体に別途出玉が乗るわけではない）。
// machineValidator.jsのballsチェックが明示的な0を許可するようになった
// （e-nanatai3.js等と同じ改訂）ため、通常時からの直行・復讐RUSH初回・
// 復讐RUSH2回目以降のいずれの「運命分岐へ」の枝もballs: 0に修正した。
//
...
PachiSim.machineRegistry.register({
  ...
  rules: [
    ...
    "RUSH突入率：約51%",
    "通常時の大当り振り分け（ヘソ入賞時）：約400個で通常のままが49%、約400個で復讐RUSHが50%、出玉無しで運命分岐直行が1%",
    "復讐RUSH（初回）中の当選：必ず出玉無しで運命分岐へ突入",
    "復讐RUSH（2回目以降）中の当選振り分け：10R・約1500個で継続が48%、出玉無しで運命分岐へが52%",
