...
// より高い）
//
// 【LT後半のLASTATTACKについて（追加確認）】
// 依頼者から追加で提供されたp-town.dmm.comのスクリーンショット（LT前半→LT後半→
// LASTATTACKのフロー図）により、LT後半にもBURST BATTLEと同一構造の「2回転
// LASTATTACK（実質大当り確率1/36.7）」が転落＆残り保留消化後に存在することが
// 確認できた（数値も1/36.7でBURST BATTLE側と同じ）。そのためBURST BATTLEと同じ
// 「残保留4個+LASTATTACK2回＝計6回」のresidualAttemptsをLT後半にも適用する。
// ただし計算すると64.2%+(1-64.2%)×[1-(1-1/36.7)^6]≈69.7%までしか届かず、
// 公表の約78%にはなお届かない。78%まで届かせるには残保留が約15～16個相当
// （LASTATTACK2回を含め計17.6回）必要という計算になり、他機種で一貫している
// 「残保留4個」という慣例から大きく外れるため、その数値は採用しない。
// つまりLASTATTACKの存在自体はスクリーンショットで裏付けが取れたが、78%との
...
PachiSim.machineRegistry.register({
  ...
  rules: [
    ...
    "BURST BATTLE：転落式（転落確率約1/9.4、残保留4個+LASTATTACK2回で突破率約33%）。突破後はLT（OVER HEAVEN）へ",
    "LT：転落式（転落確率約1/65.8、残保留4個+LASTATTACK2回で継続率約69.7%、公表約78%とはなお差あり）。継続中はOVER HEAVEN⇔後半パートを行き来する",
