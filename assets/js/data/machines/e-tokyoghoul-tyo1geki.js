...
//
// ヘソ入賞時（特図1・通常時、図柄揃い、大当り確率約1/999）の大当り振り分け:
...
// 2R・約300個、消化後の移行先は「昇格発生：約7500個獲得後にHYPER喰種RUSH突入／
// 昇格非発生：通常時へ移行」で、「大半は通常時へ移行するが、消化中に昇格演出が
// 発生すれば約7500個を獲得してHYPER喰種RUSHに突入」と説明されている）が単独の
// 表で紹介されている。ただし円グラフ（通常時の図柄揃い時の出玉振り分け確率）は
// 「10R大当り×2＝約3000個(50%)／10R大当り×5＝約7500個(50%)」の2択のみで、
// 喰種チャージ単体の昇格率は「大半は通常へ」という定性的な表現のみで、数値としては
// 公開されていない。
// このシミュレーターでは、数値の裏取りができるこの円グラフの2択をそのまま実装し、
// 喰種チャージは「通常時大当りの一部がそう見える」という演出上の呼び分けとして
// 扱う（e-gundamseed-climax.jsのSEEDチャージと同じ扱い）。喰種チャージの昇格率を
// 単独で数値化して実装すると未公開の数値を作ることになるため、あえて分けていない。
//
// 【出玉は「実獲得個数」を採用】
...
PachiSim.machineRegistry.register({
  ...
  rules: [
    "通常時大当り確率：図柄揃い約1/999（この機種の売り。喰種チャージ約1/538.8との合算では約1/349.9）",
